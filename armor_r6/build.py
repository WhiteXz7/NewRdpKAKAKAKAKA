...
from engine.animation import (make_walk, make_run, make_idle, make_attack, make_block,
                              write_bvh, write_lua_animations, FPS)
...
def render_grid(variants_parts, path):
    """Grade (3 colunas) com vista 3/4 de cada variante."""
    w, h = 480, 640
    cols = 3
    n = len(variants_parts)
    rows = (n + cols - 1) // cols
    r = Renderer(w * cols, h * rows)
...
def main():
    clips = {"idle": make_idle(), "walk": make_walk(), "run": make_run(),
             "attack": make_attack(), "block": make_block()}
    ...
    clip_list = [("idle", clips["idle"], "Idle"),
                 ...
                 ("attack", clips["attack"], "Action"),
                 ("block", clips["block"], "Action")]
