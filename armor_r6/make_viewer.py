...
from engine.animation import make_idle, make_walk
...
def main():
    ...
    modes = {"turntable": 16, "idle": 16, "walk": 16}
    ...
    for name in A.VARIANTS:
        ...
        for mname, clip in (("idle", idle), ("walk", walk)):
            n = modes[mname]
            for i in range(n):
...
<div class="wrap">
  <aside class="side">
    <div class="group">
      <h2>Variante</h2>
      <button class="btn active" data-v="european">Europeia (full plate)</button>
      <button class="btn" data-v="japanese">Samurai (ō-yoroi)</button>
      <button class="btn" data-v="persian">Chahar-aina (4 espelhos)</button>
      <button class="btn" data-v="ming">Brigandine Ming</button>
    </div>
    <div class="group">
      <h2>Visualização</h2>
      <button class="btn active" data-m="turntable">🔄 Turntable 360°</button>
      <button class="btn" data-m="idle">🧍 Idle (respiração)</button>
      <button class="btn" data-m="walk">🚶 Walk (caminhada)</button>
    </div>
...
<script>
const variants = ['european','japanese','persian','ming'];
const modes = ['turntable','idle','walk'];
...
function setMode(nm){ m = nm; i = 0; update(); document.querySelectorAll('[data-m]').forEach(b=>b.classList.toggle('active', b.dataset.m===m)); }
function setVariant(nv){ v = nv; i = 0; update(); document.querySelectorAll('[data-v]').forEach(b=>b.classList.toggle('active', b.dataset.v===v)); }
