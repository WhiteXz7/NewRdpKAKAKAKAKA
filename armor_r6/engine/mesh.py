...
def rounded_rect_ring(hx, hy, r, n=32):
    """Contorno de um retângulo arredondado (no plano XZ, CCW visto de cima).

    hx, hy = meias-extensões em x/z; r = raio do canto. Devolve lista (x, z).
    """
    bx, by = hx - r, hy - r
    ring = []
    for i in range(n):
        th = 2 * math.pi * i / n
        px, py = math.cos(th) * (hx + 1.0), math.sin(th) * (hy + 1.0)  # fora, garantido
        qx, qy = abs(px) - bx, abs(py) - by
        if qx <= 0 and qy <= 0:
            # (não ocorre: ponto gerado está fora)
            px, py = math.copysign(hx, px), math.copysign(hy, py)
        elif qx > 0 and qy > 0:
            gl = math.hypot(qx, qy) or 1.0
            px = math.copysign(bx + qx / gl * r, px)
            py = math.copysign(by + qy / gl * r, py)
        elif qx > 0:
            px = math.copysign(hx, px)     # lado x
        else:
            py = math.copysign(hy, py)     # lado z
        ring.append((px, py))
    return ring


def box_tube(xr, zr, corner, y0, y1, name="box_tube", material="steel",
             n=28, cap_bottom=True, cap_top=True):
    """Prisma extrudado em Y com seção retangular arredondada (em XZ).

    Usado para a roupa de base (gambeson/malha) que envolve o bloco do R6
    sem deixar os cantos do bloco aparecerem.
    """
    ring = rounded_rect_ring(xr, zr, corner, n)
    m = Mesh(name, material)
    bot = [m.add_vert((x, y0, z)) for (x, z) in ring]
    top = [m.add_vert((x, y1, z)) for (x, z) in ring]
    for i in range(n):
        j = (i + 1) % n
        m.add_face((bot[i], bot[j], top[j], top[i]))
    if cap_bottom:
        c = m.add_vert((0.0, y0, 0.0))
        for i in range(n):
            j = (i + 1) % n
            m.add_face((c, bot[j], bot[i]))
    if cap_top:
        c = m.add_vert((0.0, y1, 0.0))
        for i in range(n):
            j = (i + 1) % n
            m.add_face((c, top[i], top[j]))
    return m
...
MTL_PALETTE = {
    ...
    "fur":          ((0.50, 0.40, 0.28), (0.08, 0.07, 0.06), 5.0),
