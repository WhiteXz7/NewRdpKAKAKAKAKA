...
from .mesh import (Mesh, box, tube, lathe, dome, thicken_surface,
                   rounded_rect_plate, fix_orientation)
...
def underlayer_parts(material="mail"):
    ...
    """
    parts = []
    t = tube(1.12, 1.12, 1.02, 3.02, segments=24, name="under_torso", material=material)
    t.scale((1.0, 1.0, 0.6))
    parts.append((_out(t), "Torso"))
    for side in (1, -1):
        tag = "R" if side > 0 else "L"
        a = tube(0.62, 0.62, 1.05, 2.95, segments=20, name="under_arm_" + tag, material=material)
        a.translate((ARM_R * side, 0, 0))
        parts.append((_out(a), "Right Arm" if side > 0 else "Left Arm"))
        l = tube(0.62, 0.62, 0.02, 1.98, segments=20, name="under_leg_" + tag, material=material)
...
VARIANTS = ["european", "japanese", "persian", "ming"]
